...
def _compute_priors(train_df: pd.DataFrame, target: str
    ...
    """Compute leakage-safe per-cell / per-LSOA violent-rate priors on the
    training rows only.
    ...
    priors["violent_rate_grid"] = train_df.groupby("grid_id")[target].mean()
    if "lsoa_code" in train_df.columns:
        priors["violent_rate_lsoa"] = (
            train_df.groupby("lsoa_code")[target].mean()
        )
    priors["global_rate"] = pd.Series(
        [float(train_df[target].mean())], index=["global_rate"]
    )
    return priors
...
def build_features(df: pd.DataFrame,
    ...
    """
    target = config_v1.CLASSIFIER.target
    ...
    X["grid_freq"] = df["grid_id"].map(priors["grid_freq"]).fillna(0.0).to_numpy()
    X["violent_rate_grid"] = (
...
def _make_models() -> Dict[str, Pipeline]:
    rs = config_v1.CLASSIFIER.random_state
    cw = config_v1.CLASSIFIER.class_weight
    return {
        ...
        "random_forest": Pipeline([
            ("clf", RandomForestClassifier(
                n_estimators=config_v1.CLASSIFIER.rf_n_estimators,
                class_weight="balanced_subsample",
                n_jobs=-1, random_state=rs)),
        ]),
        "svm": Pipeline([
            ("scaler", StandardScaler()),
            ("clf", SVC(C=config_v1.CLASSIFIER.svm_c,
                        probability=True,
                        class_weight=cw,
                        random_state=rs)),
        ]),
...
def fit_all(df: pd.DataFrame) -> List[ClfResult]:
    ...
    train_idx, test_idx = train_test_split(
        all_idx,
        test_size=config_v1.CLASSIFIER.test_size,
        random_state=rs,
        stratify=y_full,
    )
    ...
    for name, pipe in _make_models().items():
        ...
        if name == "svm":
            Xt, yt = _subsample(
                X_train, y_train,
                n=config_v1.CLASSIFIER.svm_subsample,
                random_state=rs,
            )
...
def save(results: List[ClfResult],
         path: Path = config_v1.MODEL_DIR / "classifiers.pkl") -> None:
    rows = []
    models = {}
    for r in results:
        rows.append({"model": r.name, **r.metrics})
        models[r.name] = r.model
    pd.DataFrame(rows).to_csv(config_v1.DATA_DIR / "classifier_metrics.csv",
                              index=False)
